 ...
 */
public class RuleToHtmlTextVisitor extends ANTLRv4ParserBaseVisitor<String> {

	//private static final String ALT_PREFIX = "<b>...</b>";
	public static final String ALT_PREFIX = "";
	public static final String NEWLINE = "\n";
	public static final int INDENT_START_IDX = 0;
    ...
    public String visitParserRuleSpec(@NotNull ParserRuleSpecContext ctx) {
        ...
        buf.append("<span id=\"" + ruleName + "\">");
        buf.append("<pre xml:space=\"preserve\">");
		...
		System.out.println("RULE: " + ruleName + " (" + getDepth(ctx) + ")");
    ...
    public String visitLexerRule(@NotNull LexerRuleContext ctx) {
        ...
        String rule = this.visitLexerRuleBlock(ctx.lexerRuleBlock());
    ...
    public String visitRuleAltList(@NotNull RuleAltListContext ctx) {
        ...
        //StringBuilder builder = new StringBuilder("Choice(0, ");
        StringBuilder builder = new StringBuilder("");
	    System.out.println("RULE ALT: " + ctx.getText() + "[" + ctx.depth() + "] (" + getDepth(ctx) + ")");
	    if (ctx.getText().startsWith("(K_CONSTRAINTname)?(K_PRIMARYK_KEY(K_ASC|K_DESC)?conflict_clauseK_AUTOINCREMENT")) {
			int foo = 0;
			foo++;
	    }
        ...
        if (alternatives.size() > 1) {
    		...
    		for (int i = 0; i < alternatives.size(); i++) {
    			...
    			if (token.contains("<b>DEFAULT</b>")) {
    				int foo = 0;
    				foo++;
     			}
       			System.out.println("TOKEN1: '" + token + "' [" + ctx.depth() + "] (" + getDepth(ctx) + ")");
   			    if (i > 0) {
    				builder.append(NEWLINE);
    				for (int j = INDENT_START_IDX; j < alternative.depth()/2; j++)
    					builder.append("&nbsp;");    					
    				builder.append(ALT_PREFIX);
    				builder.append("&nbsp;");
    				builder.append("<b>|</b>");
    				builder.append("&nbsp;");
    ...
    private int getDepth(ParserRuleContext ctx) {
    	int result = 0;
    ...
    public String visitAltList(@NotNull AltListContext ctx) {
        StringBuilder builder = new StringBuilder("");
 	    System.out.println("ALT: " + ctx.getText() + "[" + ctx.depth() + "] (" + getDepth(ctx) + ")");
        ...
        if (alternatives.size() > 1) {
    		...
    		for (int i = 0; i < alternatives.size(); i++) {
    			...
    			String token = this.visitAlternative(alternative);
       			System.out.println("TOKEN2: '" + token + "' [" + ctx.depth() + "] (" + getDepth(ctx) + ")");
        			//System.out.println(token + " (" + getDepth(ctx) + ")");
    			if (i > 0) {
    				if (!allTerminal) {
       				    builder.append(NEWLINE);
        				for (int j = INDENT_START_IDX; j < alternative.depth()/2; j++)
...
//                    "(" + this.visitBlock(ctx.block()) + ")";
    ...
    public String visitNotSet(@NotNull NotSetContext ctx) {
        if (ctx.setElement() != null) {
        	return this.visitSetElement(ctx.setElement());
        ...
        switch (node.getSymbol().getType()) {
            ...
            case ANTLRv4Lexer.TOKEN_REF:
             ...
             case ANTLRv4Lexer.RULE_REF:
            	token = node.getText();
             	return "<a href=\"" + token + ".html\" target=\"_top\">" + token + "</a>"; 
            default:
            	token = node.getText();
             	return token;
         }
